...
2. **반드시 이용자 본인이 요청**: SSAPI는 이용을 원하시는 최종 사용자(서버장, 콘텐츠 주최 스트리머, 해당 서비스/콘텐츠의 공식 운영진)가 심사를 요청해야 합니다. 아래와 같은 요청은 거절됩니다:
   * 외주 용역을 받은 개발/제작팀에서의 요청
   * 스트리머의 요청을 받은 시청자의 요청
3. **개인정보 보호**:
   * **동의 확인**: SSAPI는 수집하려는 스트리머들로부터 명시적 또는 묵시적 동의를 받은 경우에 한해 데이터를 제공합니다.
     * **허용**: 마인크래프트 서버 상 API 시스템 구축 (스트리머들의 묵시적 동의가 있다고 판단됨)
     * **불가**: 별풍선 통계사이트 제작 (스트리머들의 명시적 동의가 없는 경우)
   * **데이터 저장 주기**: 제공받은 데이터는 SSAPI의 데이터 보관 주기보다 오래 저장해서는 안됩니다.
